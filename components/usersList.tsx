...
// <Button onClick={deleteStatus} className="bg-red-300 cursor-pointer hover:bg-red-400 text-sm text-green-900 font-bold">{showDeleteBtn ? " Disable Delete " : "Enable Delete"}</Button>
// </div>
...
async function serverUpdate(userId: string, status: "ACTIVE" | "INACTIVE" | "DORMANT") {
...
export default function UsersTable({ data, deleteType }: usersTableProps) {
  ...
  <TableCell className="hidden sm:flex flex-col sm:flex-row md:flex-row gap-1">
    {showDeleteBtn && <DeleteDialog deleteType={deleteType} id={user?.id} />}
    <Link
      href={`/admin/users/${user?.id}`}
      className="bg-blue-700 p-1 rounded text-white hover:bg-blue-800 active:bg-blue-900 dark:bg-blue-600 dark:hover:bg-blue-500"
    >
