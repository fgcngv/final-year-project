...
export default function FarmerOrdersProducts({
  ...
  className="border-emerald-600 text-emerald-700 hover:bg-emerald-600 hover:text-white dark:border-emerald-500 dark:text-emerald-400"
>
          {!actionBtn ? " View Products" : " Hide Products"}
        </Button>
  ...
  variant={
    product.status === "ACTIVE"
      ? "default"
      : product.status === "DORMANT" || product.status === "PAUSED"
      ? "secondary"
      : "outline"
  }
>
  ...
  function highlightText(text: string) {
    ...
    return parts.map((part, idx) =>
      regex.test(part) ? (
        <mark key={idx} className="bg-amber-300/70 text-black dark:bg-amber-500/40 dark:text-white rounded px-1">
          {part}
        </mark>
      ) : (
        part
